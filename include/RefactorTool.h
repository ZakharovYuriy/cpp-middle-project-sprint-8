...
// RefactorHandler (наследник MatchFinder::MatchCallback) содержит логику, что делать при нахождении совпадений в AST.
...
public:
    ...
    // Получаете из Result узлы, найденные матчером (Result.Nodes.getNodeAs<T>()).
    // Проверяете, что узел относится к основному файлу.
    // Используете Rewriter для внесения изменений в код (например, добавления virtual или override).
    // Именно здесь описывается конкретная трансформация кода.
...
// (наследник ASTConsumer) отвечает за регистрацию всех матчеров
// ASTConsumer управляет поиском нужных узлов AST и передаёт результаты в обработчик.
...
// (наследник ASTFrontendAction) запускает процесс анализа для каждого файла в проекте.
// Здесь создаётся ASTConsumer (наш ComplexConsumer) и настраивается Rewriter,
// который будет использоваться для внесения изменений.
class CodeRefactorAction : public clang::ASTFrontendAction {
public:
    // Returns our ASTConsumer per translation unit.
    virtual std::unique_ptr<clang::ASTConsumer> CreateASTConsumer(clang::CompilerInstance &CI,
                                                                  clang::StringRef file) override;
    // инициализирует Rewriter
    virtual bool BeginSourceFileAction(clang::CompilerInstance &CI) override;
    // после обработки файла сохраняет изменения на диск через Rewriter.overwriteChangedFiles()
